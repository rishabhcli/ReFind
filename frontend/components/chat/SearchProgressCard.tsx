...
import { PlatformLogo } from "@/components/icons/PlatformIcons";

const SOURCE_CONFIG: Record<string, { label: string; sourceKey: string }> = {
  search_ebay: { label: "eBay", sourceKey: "ebay" },
  search_mercari: { label: "Mercari", sourceKey: "mercari" },
  search_craigslist: { label: "Craigslist", sourceKey: "craigslist" },
  search_goodwill: { label: "Goodwill", sourceKey: "goodwill" },
  search_offerup: { label: "OfferUp", sourceKey: "offerup" },
  search_facebook: { label: "Facebook", sourceKey: "facebook" },
  search_facebook_marketplace: { label: "Facebook", sourceKey: "facebook" },
  search_poshmark: { label: "Poshmark", sourceKey: "poshmark" },
  browser_enricher: { label: "Enriching top deals", sourceKey: "browser_enricher" },
};
...
function SearchToolBadge({
  ...
  return (
    <ToolScrollRail animate={false} className="my-1">
      ...
      >
        <PlatformLogo source={config.sourceKey} size={14} />
        {isRunning ? (
