module.exports = [
"[project]/node_modules/@workos-inc/node/lib/webapi-Cj8QQOwM.mjs [middleware] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
function decode(input) {
    if (Uint8Array.fromBase64) return Uint8Array.fromBase64(typeof input === "string" ? input : decoder.decode(input), {
        alphabet: "base64url"
    });
    let encoded = input;
...
async function cbcKeySetup(enc, cek, usage) {
    ...
    return {
        encKey: await crypto.subtle.importKey("raw", cek.subarray(keySize >> 3), "AES-CBC", false, [
            usage
        ]),
...
async function gcmEncrypt(enc, plaintext, cek, iv, aad) {
    let encKey;
    if (cek instanceof Uint8Array) encKey = await crypto.subtle.importKey("raw", cek, "AES-GCM", false, [
        "encrypt"
    ]);
    else {
...
function getCryptoKey$1(key, alg, usage) {
    if (key instanceof Uint8Array) return crypto.subtle.importKey("raw", key, "AES-KW", true, [
        usage
    ]);
...
function getCryptoKey(key, alg) {
    if (key instanceof Uint8Array) return crypto.subtle.importKey("raw", key, "PBKDF2", false, [
        "deriveBits"
    ]);
...
async function normalizeKey(key, alg) {
    ...
    if (isKeyObject(key)) {
        ...
        return handleJWK(key, key.export({
            format: "jwk"
        }), alg);
    }
    if (isJWK(key)) {
        ...
        case "ECDH-ES+A256KW":
            {
                ...
                else ephemeralKey = (await crypto.subtle.generateKey(key.algorithm, true, [
                    "deriveBits"
                ])).privateKey;
...
function validateCrit(Err, recognizedDefault, recognizedOption, protectedHeader, joseHeader) {
    ...
    let recognized;
    if (recognizedOption !== void 0) recognized = new Map([
...
//# sourceMappingURL=node_modules_%40workos-inc_node_lib_webapi-Cj8QQOwM_mjs_0wjautl._.js.map